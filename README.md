...
- スピーディーに検証するための簡易的な構成であり、本番環境に適用できるものではない。
...
- 公式の手順になるべく近づけるため、サブネット1,2はパブリックIPアドレスを付与する設定のままとしている。
...
5. 「次へ」をクリック
6. 「タグ - オプション」で必要に応じて、作成するリソース全体に付与するタグのキー、値を入力する。
7. 「AWS CloudFormation によって IAM リソースがカスタム名で作成される場合があることを承認します。」にチェックをつける。
8. 「次へ」をクリック
9. ここまでの設定内容が正しく入力されていることを確認。
10. 「送信」をクリック
11. スタックの作成が開始される。作成したスタックのステータスが「CREATE_COMPLETE」となれば完了。
12. EC2の画面に遷移し、作成したFleet ManagerのVMのプライベートIPv4アドレスをコピーして、保存しておく。
